...
export const updateAdvertisement = async (advertId = null, value) => {
  ...
  checkTypeOfAdvertisement(value);
  try {
    let update_doc = await updateDoc(
      doc(db, doc_collection, advertId),
      preSaveOrUpdateAClass(value)
    );
...
export const getAdvertisementById = async (advertId = null) => {
  ...
  if (data.exists()) {
    ...
    return {
      success: true,
      data: Object.assign(checkAdvertisemntType(data.data()), {
        ...data.data(),
        _id: data.id,
      }),
    };
...
export const getAllAdvertisement = async () => {
  ...
  if (docs.size !== 0) {
    docs.forEach((data) => {
      ads.push(
        Object.assign(checkAdvertisemntType(data.data()), {
          ...data.data(),
          _id: data.id,
        })
      );
